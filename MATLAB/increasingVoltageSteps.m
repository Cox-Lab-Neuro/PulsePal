...
amps = [1, 0:0.1:5]; %add one at the beginning to know when trigger starts
% pulse = repmat(amps, 8);
% pulse = reshape(pulse, [], 1);
pulse = amps;
...
ProgramPulsePalParam(1, 10, 10); % set pulse train duration to 10s
ProgramPulsePalParam(1, 128, 0); % set trigger mode to pulse-gated

% Send waveform
ConfirmBit = SendCustomWaveform(1, 0.5, pulse);